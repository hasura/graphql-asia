...
const IndivTalksPage = (props) => {
  ...
  const renderCopyIcon = () => {
    if (isCopied) {
      return (
        <div
          className={'copiedWrapper ' + (!props.isLightMode ? 'copiedDarkMode' : 'copiedLightMode')}
        >
          Copied
        </div>
      );
    ...
    <Fragment>
      <div className="detailsPageWrapper sectionWrapper removePaddTop">
        ...
        {props.currentDetailsPage.takeaways ? (
          <Fragment>
            <div className="articleSectionTitle fontBold">Talk Takeaways</div>
          ...
          <Fragment>
            {props.currentDetailsPage.contents.map((desc, index) => {
              return (
                <Fragment key={index}>
                  {desc.title ? (
                    <div className="articleSectionTitle fontBold">{desc.title}</div>
                  ) : null}
                  <div key={index} className="articleDesc pb-40">
                    {desc.desc ? desc.desc : null}
                    {desc.list ? (
              ...
              return (
                <div key={index} className="speakerDetails">
                  <div className="speakersProfile">
                    ...
                    <div className="pl-40">
                      <div className="commonLink">{speakers.speakersName}</div>
                      <div className="articleDesc fontBold">{speakers.speakersDesignation}</div>
                      {speakers.speakersBio ? (
                        <div className="articleDesc">{speakers.speakersBio}</div>
                      ) : null}
                    </div>
                  </div>
                </div>
        ...
        <div className="shareWrapper">
          ...
          <div className="shareIconWrapper">
            <CopyToClipboard
              text={`https://graphql.asia/${props.currentDetailsPage.url}`}
              onCopy={onCopy}
            >
            ...
            <a
              className="shareIcon"
              href={`https://wa.me/?text=https://graphql.asia/${props.currentDetailsPage.url}`}
            ...
            <a
              className="shareIcon"
              href={`https://twitter.com/intent/tweet?&text=${props.currentDetailsPage.title}&url=https://graphql.asia/${props.currentDetailsPage.url}`}
              target="_blank"
            ...
            <a
              className="shareIcon"
              href={`http://www.linkedin.com/shareArticle?mini=true&url=https://graphql.asia/${props.currentDetailsPage.url}&title=${props.currentDetailsPage.title}&summary=${props.currentDetailsPage.title}&source=https://graphql.asia/${props.currentDetailsPage.url}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              <IconHover
